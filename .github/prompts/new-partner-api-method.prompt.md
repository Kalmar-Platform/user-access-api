...
description: 'Generate a new API method for the Partner API which is part of the Subscription API project'
---

 Your goal is to implement a new API method ${input:methodName} in the Suite API of the Subscription API project following the guidelines provided in the instructions files.
 ...
 - The `Partner` API is part of the `Subscription` database, so you will need to create or update the `Partner` table in the `Subscription` database.
 ...
 - A `Partner` only exists in the `Subscription` database, so any search for a `Partner` should be done in the `Subscription` database.
 ...
 - In this case you need to use the `CountryGateway` interface to fetch the `Country` data from the `Subscription` database.
