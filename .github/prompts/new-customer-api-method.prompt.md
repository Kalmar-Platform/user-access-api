...
---
 ...
 - A `Customer` table exists in both the `UserAccess` and `Subscription` databases, so that will need to created or updated in both databases.
 ...
 - Unless otherwise specified, the `Country` data should be fetched from the `UserAccess` database through the existing `CountryGateway` interface.
