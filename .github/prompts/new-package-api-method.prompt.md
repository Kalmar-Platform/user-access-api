...
description: 'Generate a new API method for the Package API which is part of the Subscription API project'
---
 Your goal is to implement a new API method ${input:methodName} in the Package API of the Subscription API project following the guidelines provided in the instructions files.
 ...
 When implementing the method, besides the guidelines in all the instruction files you also need to consider the particularities of the Package API:
  - The `Package` API is part of the `Subscription` database, so you will need to create or update the `Package` table in the `Subscription` database.
