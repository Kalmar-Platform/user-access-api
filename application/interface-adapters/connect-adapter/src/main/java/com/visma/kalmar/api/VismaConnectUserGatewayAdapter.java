...
import com.visma.kalmar.api.connect.dto.ConnectEmailActivationRequest;
...
public class VismaConnectUserGatewayAdapter implements VismaConnectUserGateway {
  ...
  public void unlinkClient(UUID userId) {
    webClient
        .put()
        .uri("/v1.0/users/{user_id}/unlinkClient", userId)
        .retrieve()
        .onStatus(
            status -> status.is4xxClientError() || status.is5xxServerError(),
            clientResponse ->
  ...
  public void sendEmailActivation(
      ...
      String state,
      String recipient) {
    var request = new ConnectEmailActivationRequest(applicationName, redirectUri, state, recipient);

    webClient
        .post()
        .uri("/v1.0/emails/{user_id}/welcome", connectUserId)
        .contentType(MediaType.APPLICATION_JSON)
        .bodyValue(request)
        .retrieve()
        .onStatus(
